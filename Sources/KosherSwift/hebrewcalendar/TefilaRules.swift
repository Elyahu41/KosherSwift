 ...
 * changes to daily <em>tefila</em> / prayers, based on the Jewish calendar. self is mostly useful for use in
 ...
 * recited that day. There are many settings in self class to cover the vast majority of <em>minhagim</em>, but
 ...
 * (Medzhybizh) celebrated on 11 {@link JewishCalendar#TEVES <em>Teves</em>} or <a href=
 * "https://en.wikipedia.org/wiki/Second_Purim#Purim_Saragossa_(18_Shevat)">Purim Saragossa</a> celebrated on
 * the (17th or) 18th of {@link JewishCalendar#SHEVAT <em>Shevat</em>} are not (and likely will not be) supported by
 * self class.
...
public class TefilaRules {
     ...
     * (<em>Sheailas Geshamim</em>). In Israel self is the 7th day of {@link JewishCalendar#CHESHVAN <em>Marcheshvan</em>}.
     * Outside Israel recitation starts on the evening of December 4th (or 5th if it is the year before a civil leap year)
     * in the 21st century and shifts a day forward every century not evenly divisible by 400. self method will return true
     * if <em>vesein tal umatar</em> on the current Jewish date that starts on the previous night, so Dec 5/6 will be
     * returned by self method in the 21st century. <em>vesein tal umatar</em> is not recited on <em>Shabbos</em> and the
     * start date will be delayed a day when the start day is on a <em>Shabbos</em> (self can only occur out of Israel).
     *
     * @param jewishCalendar the Jewish calendar day.
     *
     ...
     * <em>Sheailas Geshamim</em>). In Israel self is the 7th day of {@link JewishCalendar#CHESHVAN
     ...
     * (self can only occur out of Israel).
     *
     * @param jewishCalendar the Jewish calendar day.
     *
    ...
    /**
     * Returns if <em>Vesein Tal Umatar Livracha</em> (<em>Sheailas Geshamim</em>) is recited. self will return
    ...
    /**
     * Returns if <em>Vesein Beracha</em> is recited. It is recited from 15 {@link JewishCalendar#NISSAN <em>Nissan</em>} to the
     ...
     * {@link JewishCalendar#TISHREI <em>Tishrei</em>}.
     *
     * @param jewishCalendar the Jewish calendar day.
     ...
     * {@link JewishCalendar#NISSAN <em>Nissan</em>}.
     *
     * @param jewishCalendar the Jewish calendar day.
    ...
    /**
     * Returns if <em>Mashiv Haruach Umorid Hageshem</em> is recited. self period starts on 22 {@link
     * JewishCalendar#TISHREI <em>Tishrei</em>} and ends on the 15th day of {@link JewishCalendar#NISSAN <em>Nissan</em>}.
     *
     * @param jewishCalendar the Jewish calendar day.
     ...
     * recited. self period starts on the 15th day of {@link JewishCalendar#NISSAN <em>Nissan</em>} and ends on 22 {@link
    ...
    /**
     * Returns if <em>Hallel</em> is recited on the day in question. self will return true for both <em>Hallel shalem</em>
     ...
     * JewishCalendar#ADAR <em>Adar</em>} (on a non-leap year, or {@link JewishCalendar#ADAR_II <em>Adar II</em>} on a leap year). Some
     * <em>chasidishe</em> communities do not recite <em>tachanun</em> during self period. See the <a href=
     ...
     *         through the 17th of {@link JewishCalendar#ADAR <em>Adar</em>} (on a non-leap year), or {@link JewishCalendar#ADAR_II
     ...
     * <em>chasidishe</em> communities do not recite <em>tachanun</em> during self week. See Minhag Yisrael Torah 131:Iyar.
     * @return If <em>tachanun</em> is set to be recited during the <em>sefira</em> week of <em>Hod</em> (14 - 20 {@link
     *         JewishCalendar#IYAR <em>Iyar</em>}, or the 29th - 35th of the {@link JewishCalendar#getDayOfOmer() <em>Omer</em>}).
     * @see #setTachanunRecitedWeekOfHod(boolean)
     */
     ...
     * Choshen Mishpat 77 writes that self is the <em>minhag</em> in Ashkenaz. The Shaarei Teshuva 131:19 quotes the Sheyarie Kneses
     * Hagdola who also states that it should not be recited. The Aderes wanted to institute saying <em>tachanun</em> during self
     * period, but was dissuaded from self by Rav Shmuel Salant who did not want to change the <em>minhag</em> in Yerushalayim.
     * The Aruch Hashulchan is of the opinion that that self <em>minhag</em> is incorrect, and it should be recited, and The Chazon
     * Ish also recited <em>tachanun</em> during self period. See the Dirshu edition of the Mishna Berurah for details.
     * @return If <em>tachanun</em> is set to be recited at the end of {@link JewishCalendar#TISHREI <em>Tishrei</em>}.
    ...
    /**
     * Is <em>tachanun</em> set to be recited during the week after <em>Shavuos</em>. self is the opinion of the Pri
     * Megadim quoted by the Mishna Berurah. self is since <em>karbanos</em> of <em>Shavuos</em> have <em>tashlumim</em>
     * for 7 days, it is still considered like a <em>Yom Tov</em>. The Chazon Ish quoted in the Orchos Rabainu vol. 1
     * page 68 recited <em>tachanun</em> during self week.
     *
    ...
    /**
     * Is <em>tachanun</em> is set to be recited on the 13th of {@link JewishCalendar#SIVAN <em>Sivan</em>} (<a href=
     * "https://en.wikipedia.org/wiki/Yom_tov_sheni_shel_galuyot"><em>Yom Tov Sheni shel Galuyos</em></a> of the 7th
     * day) outside Israel. self is brought down by the Shaarie Teshuva 131:19 quoting the <a href=
     * "https://hebrewbooks.org/pdfpager.aspx?req=41295&st=&pgnum=39">Sheyarei Kneses Hagedola 131:12</a>that
     * <em>tachanun</em> should not be recited on self day. Rav Shlomo Zalman Orbach in Halichos Shlomo on
     ...
     *          JewishCalendar#SIVAN <em>Sivan</em>} out of Israel. Both {@link #isTachanunRecitedShacharis(JewishCalendar)} and
     ...
     * states that <em>tachanun</em> should not be recited. The Aruch Hashulchan states that self is the minhag of the
    ...
    /**
     * Is <em>tachanun</em> set to be recited on 15 {@link JewishCalendar#IYAR <em>Iyar</em>} (<em>sfaika deyoma</em> of {@link
     * JewishCalendar#PESACH_SHENI <em>Pesach Sheni</em>}) out of Israel. If {@link #isTachanunRecitedPesachSheni()} is
     * <code>true</code> self will be ignored even if <code>false</code>.
     *
     * @return if <em>tachanun</em> is set to be recited on 15 {@link JewishCalendar#IYAR <em>Iyar</em>}  (<em>sfaika deyoma</em>
     *          of {@link JewishCalendar#PESACH_SHENI <em>Pesach Sheni</em>} out of Israel. If
     *          {@link #isTachanunRecitedPesachSheni()} is <code>true</code> self will be ignored even if <code>false</code>.
     * @see #setTachanunRecited15IyarOutOfIsrael(boolean)
     * @see #setTachanunRecitedPesachSheni(boolean)
     * @see #isTachanunRecitedPesachSheni()
     */
     ...
     *
     * @param tachanunRecited15IyarOutOfIsrael if <em>tachanun</em> should be recited on the 15th of {@link JewishCalendar#IYAR
     ...
     * JewishCalendar#ADAR <em>Adar</em>} on a non-leap-year or {@link JewishCalendar#ADAR_II <em>Adar II</em>} on a
     * leap year to the end of the month. Some <em>chasidishe</em> communities do not say <em>tachanun</em>
     * during self week. See <a href="https://hebrewbooks.org/pdfpager.aspx?req=4692&st=&pgnum=70">Darkei
     * Chaim Veshalom 191</a>.
     * @return if <em>tachanun</em> is set to be recited during the <em>Shivas Yemei Hamiluim</em>, from the 23
     *           of {@link JewishCalendar#ADAR <em>Adar</em>} on a non-leap-year or {@link JewishCalendar#ADAR_II
     ...
     * Maharich Vol 2 Seder Hanhagos Erev Shabbos</a>. self is also the <em>minhag</em> in Satmar.
