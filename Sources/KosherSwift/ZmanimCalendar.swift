 ...
 *  ``setUseElevation(useElevation:)``. This will impact sunrise and sunset-based <em>zmanim</em> such as ``getSunrise()``,
 ...
 * true it will not impact <em>zmanim</em> that are a degree-based offset of sunrise and sunset, such as
...
public class ZmanimCalendar : AstronomicalCalendar {
     ...
     * fixed offset of sunrise or sunset and <em>zmanim</em> based on a percentage of the day such as
    ...
    /**
     * Is ``getSunTransit()`` astronomical <em>chatzos</em> used for ``getChatzos()`` for enhanced accuracy. For
     ...
     * <em>chatzos</em> as opposed to 6.5 <em>shaaos zmaniyos</em> after sunrise. ``getPlagHamincha()`` <em>Plag
     ...
     * would be calculated as 3 <em>shaaos zmaniyos</em> after sunrise, but the <em>shaaos
     ...
     * 72 minutes before ``getSunrise()`` in Jerusalem <a href=
     ...
     * is 8.5&deg; below ``GEOMETRIC_ZENITH``. The <em><a href=
     ...
     * default), or elevation adjusted ``AstronomicalCalendar.getSunrise()`` if it is true. This allows relevant <em>zmanim</em>
     ...
     * or elevation adjusted ``AstronomicalCalendar.getSunrise()`` if it is true. This allows relevant <em>zmanim</em>
     * in this and extending classes (such as the ``ComplexZmanimCalendar``) to automatically adjust to the elevation setting.
     *
     ...
     * ``GEOMETRIC_ZENITH`` geometric horizon (90&deg;) after ``getSunset()``, a time that Rabbi Meir
    ...
    /**
     * Returns <em>alos</em> (dawn) based on the time when the sun is  ``ZENITH_16_POINT_1`` 16.1&deg; below the
     ...
     * takes to walk 4 <em>mil</em> at 18 minutes a mil (<em><a href="https://en.wikipedia.org/wiki/Maimonides">Rambam</a></em> and others). The sun's position at 72 minutes before ``getSunrise()`` in Jerusalem
     * on the <a href="https://kosherjava.com/2022/01/12/equinox-vs-equilux-zmanim-calculations/">around the equinox /
     * equilux</a> is 16.1&deg; below ``GEOMETRIC_ZENITH``.
     *
    ...
    /**
     * This method returns ``getSunTransit()`` Astronomical <em>chatzos</em> if the
     * ``AstronomicalCalculator``  class used supports it and
     * ``isUseAstronomicalChatzos()``  is set to <em>true</em> or the ``getChatzosAsHalfDay()``
     ...
     * zmaniyos</em> after sunrise), while the ``NOAACalculator`` calculates it more accurately as ``getSunTransit()`` astronomical
     ...
     * <em>chatzos</em> is calculated as the the midpoint between ``getSeaLevelSunrise()``  and
     ...
     * </em> as six <em>shaaos zmaniyos</em> after sunrise. There are currently two ``AstronomicalCalculator`` calculators available in the API, the
    ...
    /**
     * A generic method for calculating the latest <em>zman krias shema</em> (time to recite shema in the morning) that is 3 x
     ...
     * the day. If ``isUseAstronomicalChatzosForOtherZmanim()`` is <code>true</code>, the 3 <em>shaos zmaniyos</em> will be
     * based on 1/6 of the time between sunrise and ``getSunTransit()`` astronomical <em>chatzos</em>}. As an example, passing
     ...
     * where the start and end dates are not synchronous such as in ``ComplexZmanimCalendar.getSofZmanShmaAlos16Point1ToTzaisGeonim7Point083Degrees()`` <code>false</code> should be passed to the synchronous parameter
     ...
     *            this method.
     * @param synchronous
     *            If the <em>zman</em> has a synchronous start and end of the day. If this is <code>false</code>, using a ``isUseAstronomicalChatzosForOtherZmanim()`` makes no sense and will be ignored even if set to true, since by
    ...
    /**
     * A generic method for calculating the latest <em>zman krias shema</em> that calls ``getSofZmanShma(startOfDay:endOfDay:)``
    ...
    /**
     * This method returns the latest <em>zman krias shema</em> (time to recite shema in the morning) that is 3 x
     * ``getShaahZmanisGra()`` <em>shaos zmaniyos</em> (solar hours) after ``getSunrise()`` or
    ...
    /**
     * This method returns the latest <em>zman krias shema</em> (time to recite shema in the morning) that is 3 x
     * ``getShaahZmanisMGA()`` <em>shaos zmaniyos</em> (solar hours) after ``getAlos72()``, according to the
    ...
    /**
     * A generic method for calculating the latest <em>zman tfila</em> that calls ``getSofZmanTfila(startOfDay:endOfDay:synchronous:)``
     * passing <code>false</code> to the synchronous parameter since there is no way to know if the start and end of the day are
     * synchronous. Passing true when they are not synchronous is too much of a risk. See information on that method for more details.
     * @param startOfDay
     *            the start of day for calculating <em>zman tfilah</em>. This can be sunrise or any <em>alos</em> passed
    ...
    /**
     * This method returns the latest <em>zman tfila</em> (time to recite shema in the morning) that is 4 x
     * ``getShaahZmanisGra()`` <em>shaos zmaniyos</em> (solar hours) after ``getSunrise()`` or
     * ``getSeaLevelSunrise()`` (depending on the ``setUseElevation(useElevation:)`` setting), according
     * to the <a href="https://en.wikipedia.org/wiki/Vilna_Gaon">GRA</a>.
     * The day is calculated from ``getSeaLevelSunrise()`` to ``getSeaLevelSunrise()`` or ``getSunrise()`` to ``getSunset()`` (depending on the ``setUseElevation(useElevation:)``
     ...
     * <em><a href="https://en.wikipedia.org/wiki/Vilna_Gaon">GRA</a></em>. Alternatively, this method uses
     * ``isUseAstronomicalChatzosForOtherZmanim()`` to control if the time is based on 6.5 <em>sha'os zmaniyos</em> into the day
     * mentioned above, or as half an hour <em>zmaniyos</em> based on the second half of the day after <em>chatzos</em> (``getSunTransit()`` astronomical <em>chatzos</em> if supported by the ``AstronomicalCalculator`` calculator and
     * ``isUseAstronomicalChatzos()`` configured or ``getChatzosAsHalfDay()`` <em>chatzos</em> as half a day if not. This
    ...
    /**
     * This method returns the latest <em>mincha gedola</em>,the earliest time one can pray <em>mincha</em> that is 6.5 x
     * ``getShaahZmanisGra()`` <em>shaos zmaniyos</em> (solar hours) after ``getSunrise()`` or
     * ``getSeaLevelSunrise()`` (depending on the ``setUseElevation(useElevation:)`` setting), according
     ...
     * ``getMinchaKetana()`` <em>mincha ketana</em> while the <em>Ra"sh, Tur, GRA</em> and others are of the
     ...
     * setting).
     * TODO: Consider adjusting this to calculate the time as half an hour <em>zmaniyos</em> after either ``getSunTransit()`` astronomical <em>chatzos</em> or ``getChatzosAsHalfDay()`` <em>chatzos</em> as half a day
    ...
    /**
     * A generic method for calculating <em>samuch lemincha ketana</em> that calls ``getSamuchLeMinchaKetana(startOfDay:endOfDay:synchronous:)``
     * passing <code>false</code> to the synchronous parameter since there is no way to know if the start and end of the day are
     * synchronous. Passing true when they are not synchronous is too much of a risk. See information on that method for more details.
     * @param startOfDay
     *            the start of day for calculating <em>samuch lemincha ketana</em>. This can be sunrise or any <em>alos</em>
     ...
     * 9.5 x <em>shaos zmaniyos</em> (temporal hours) after the start of the day, calculated using the start and end
    ...
    /**
     * A generic method for calculating <em>mincha ketana</em> that calls ``getMinchaKetana(startOfDay:endOfDay:synchronous:)`` passing
     ...
     * opinion of the <em><a href="https://en.wikipedia.org/wiki/Maimonides">Rambam</a></em> and others, that is 9.5 x ``getShaahZmanisGra()`` <em>shaos zmaniyos</em> (solar hours) after ``getSunrise()`` or
     * ``getSeaLevelSunrise()`` (depending on the ``setUseElevation(useElevation:)`` setting), according
    ...
    /**
     * A generic method for calculating <em>plag hamincha</em> that calls ``getPlagHamincha(startOfDay:endOfDay:synchronous:)`` passing
     * <code>false</code> to the synchronous parameter since there is no way to know if the start and end of the day are synchronous.
     * Passing true when they are not synchronous is too much of a risk. See information on that method for more details.
     * @param startOfDay
     *            the start of day for calculating plag. This can be sunrise or any <em>alos</em> passed to this method.
    ...
    /**
     * This method returns <em>plag hamincha</em>, that is 10.75 x ``getShaahZmanisGra()`` <em>shaos zmaniyos</em>}
     * (solar hours) after ``getSunrise()`` or ``getSeaLevelSunrise()`` (depending on
     * the ``setUseElevation(useElevation:)`` setting), according to the <em><a href="https://en.wikipedia.org/wiki/Vilna_Gaon">GRA</a></em>. Plag hamincha is the earliest time that <em>Shabbos</em> can be started.
     * The day is calculated from ``getSeaLevelSunrise()`` to ``getSeaLevelSunset()`` sea level
     * sunset or ``getSunrise()`` to ``getSunset()`` (depending on the ``setUseElevation(useElevation:)``
     *
    ...
    /**
     * A method that returns a <em>shaah zmanis</em> (``getTemporalHour(Date, Date)`` temporal hour) according to
     * the opinion of the <em><a href="https://en.wikipedia.org/wiki/Vilna_Gaon">GRA</a></em>. This calculation divides
     * the day based on the opinion of the <em>GRA</em> that the day runs from from ``getSeaLevelSunrise()`` sea
     ...
     * parts with each one being a <em>shaah zmanis</em>. This method is similar to ``getTemporalHour()``, but can
     ...
     * <em>tfila</em>). Dawn for this calculation is 72 minutes before ``getSunrise()`` or ``getSeaLevelSunset()``
